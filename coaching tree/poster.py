...
RADII      = {0: 0.0, 1: 3.5, 2: 7.8, 3: 13.5}
...
def compute_positions(coaches: dict) -> dict:
    ...
    # Largest sectors first for visual balance
    all_g1_sorted = sorted(all_g1, key=lambda x: sizes[x], reverse=True)
...
def render(coaches: dict, output_base: str = "coaching_tree_poster"):
    ...
    for cid, info in coaches.items():
        ...
        # Edge labels: Gen 1 and Gen 2 only (Gen 3 too dense)
        if gen <= 2:
